...
$problem =  mysqli_real_escape_string($connn, $_POST['problem']);
 ...
 if(empty($name) || empty($model) || empty($contact) || empty($garage) || empty($problem)){
 header("Location: ../appoint.php?error=emptyfields&first=".$name."&model=".$model."&contact=".$contact."&garage=".$garage."&problem=".$problem);
 exit();}
...
mysqli_stmt_execute($stmt);}
	...
	header("Location: ../appoint.php?appointment=success");
die();}else{
