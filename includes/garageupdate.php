...
$specialChars = preg_match('@[^\w]@',$pwd);
	...
	if(empty($firstt) || empty($lastt) || empty($usernamee) || empty($contact) || empty($gname) || empty($location) || empty($services) || empty($email) || empty($pwd) || empty($pwd2)){
header("Location: ../updategarage.php?error=emptyfields&first=".$firstt."&last=".$lastt."&username=".$usernamee."&contact=".$contact."&gname=".$gname."&location=".$location."&services=".$services."&email=".$email);
exit();
}else 	if(!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/", $usernamee)){
	header("Location: ../updategarage.php?error=invalidemailusername");
	exit();
}else 	if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
	header("Location: ../updategarage.php?error=invalidemail&first=".$firstt."&last=".$lastt."&username=".$usernamee."&contact=".$contact."&gname=".$gname."&location=".$location."&services=".$services);
exit();
}
else if(!preg_match("/^[a-zA-Z0-9]*$/", $usernamee)){
	header("Location: ../updategarage.php?error=invalidusername&first=".$firstt."&last=".$lastt."&contact=".$contact."&gname=".$gname."&location=".$location."&services=".$services."&email=".$email);
exit();
}else if($pwd !== $pwd2){
	header("Location: ../updategarage.php?error=passwordcheck&first=".$firstt."&last=".$lastt."&username=".$usernamee."&contact=".$contact."&gname=".$gname."&location=".$location."&services=".$services."&email=".$email);
exit();
}else if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($pwd) < 8){
			header("Location: ../updategarage.php?error=Invalid Password&first=".$firstt."&last=".$lastt."&username=".$usernamee."&contact=".$contact."&gname=".$gname."&location=".$location."&services=".$services."&email=".$email);
exit();
