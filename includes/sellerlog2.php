...
header("Location: ../sellerlognew.php?error=emptyfields&username=".$username);
exit();
	}
	if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($passcode) < 8){
	header("Location: ../sellerlognew.php?error=Invalid Password&username=".$username);
	exit();
	}
    ...
    if ($row['username'] == $username && $row['pwd'] == $hashedPwd) {
        ...
        header("Location: ../sellerlognew.php?error=Match Not Found&username=".$username);
